...
CREATE TABLE public.data_table (column_1 int, column_2 int, column_3 int, join_id int, constraint pkey_1 primary key (join_id));
...
CREATE TABLE public.email_table (join_id int, email string, constraint fkey_1 foreign key (join_id) references public.data_table (join_id));
...
SELECT email
FROM public.data_table AS dataT
JOIN public.email_table AS emailT
 ON dataT.join_id = emailT.join_id
 AND mod(dataT.column_1, 2) = 0
 AND dataT.column_2 < dataT.column_1
 AND substring(reverse(dataT.column_3), 1, 1) = '1';
